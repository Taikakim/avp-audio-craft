...
### 2026-09-17 — "the model makes harsh timbres" is mostly the PROMPT, and the band we were fixing is the wrong band

Kim asked for a note that the model seems to create harsh timbres. The honest version is
narrower and more useful than that, so here it is with what survives measurement and what
does not.

**Where harshness concentrates.** 40 of 82 mixtape clips were flagged harsh by ear. 23 of
those 40 come from one render set — `genre_fusion_probe_local` (2026-08-21), which renders
OOD genre-FUSION prompts on post-trained medium at cfg1. I first read this as "two bad
training arms" off the filename prefix `gf2_`; that was wrong, `gf2_` is the probe set and
`gf_NN` is a prompt index, not a checkpoint. The control separates the three confounded
explanations cleanly:

    genre-fusion probe (ptm, cfg1, OOD prompts)   23/31   74.2% harsh
    OTHER ptm + cfg1 (normal prompts)             13/37   35.1% harsh
    non-ptm (base-model arms)                      4/14   28.6% harsh
    by prompt: genre_fusion 74.2% · rb_rare 55.6% · rb_common 50.0% · other 12.0%

Same model family, same cfg, double the harshness when the prompt asks for a genre blend the
model was never trained on. **So it is not "this checkpoint sounds harsh" — it is "this model
gets harsh when pushed out of distribution",** and the prompt-rarity gradient (rare 55.6% >
common 50.0% > other 12.0%) says the same thing more gently. That is a far more actionable
finding than a bad-arm verdict: it predicts harshness from the REQUEST, before rendering.

**[RULED OUT] `rms_energy_air` is the wrong instrument for this.** Flagged vs not-flagged,
per clip, on the window actually judged (0-25%, since Kim skipped ahead on clips that opened
well): six bands survive Bonferroni and every one of them is LOW — 20-49 Hz (-4.11 dB,
d=-0.89), 81-117 (-3.61, d=-0.80), 157-202 (-3.72), 252-308 (-3.50, d=-0.91), 308-371
(-3.44, d=-0.92), 518-605 (-2.75, d=-0.83). Across the entire `rms_energy_air` span
(2.5-22 kHz) **not one band is significant.** Harsh here means THIN, not bright: the highs
dominate because ~3-4 dB of low-mid body is missing. An HF cut cannot fix that and makes it
worse — which is exactly what the guided renders did (air -26.2→-29.0 dB but body
-23.9→-28.0, i.e. it dropped the mids MORE than the highs, the degenerate solution to an
absolute-energy target). Any replacement target must be LEVEL-INVARIANT, same logic as
`onset_per_beat` making the tempo shortcut unexpressible.

**Three of Kim's hypotheses, all marginal, none significant — recorded so nobody re-runs them
expecting a result.** Contour roughness RMS 2.04 vs 1.81 dB (p=0.067); peak-to-valley 11.11
vs 9.87 dB (p=0.055); z0 latent std 1.366 vs 1.252 (p=0.062, n=17/34 only). All three lean
the right way at the same marginal strength, which is more interesting than one lucky metric,
but none clears 0.05. **Crest factor REFUTES the soft-distortion hypothesis as measured**:
13.37 vs 13.03 dB, p=0.62, both in the normal 10-14 dB range for undistorted dance masters.
Crest was the wrong instrument — gentle saturation adds harmonics without crushing peak/RMS;
a THD or intermodulation measure would be the real test. Both z0 pools sit mildly above the
healthy ~1.13-1.15 reference and nowhere near the 5.6 runaway family, so "growing weights
push latents over" is consistent with these arms generally but does NOT separate harsh from
acceptable.

**Method note worth carrying.** Averaging ERB contours across clips CANCELS per-clip peaks
that sit at different frequencies — Kim's ear found +5 dB at 5.5-7.5k on one clip and +8 dB
at 1.5-4.5k on another, and cross-clip averaging leaves only the shared tilt. That is why my
first pass saw a clean tilt and missed the unevenness entirely. Per-clip roughness is the
right shape of metric; it just is not significant yet at n=40/42.

Tools: `mir/src/tools/spectral_harshness_contrast.py` (ERB contours, level-anchored on
1-3 kHz, track-relative windows, per-clip Mann-Whitney + Bonferroni, codec-limited bands
marked). Labels: G's export, 2026-09-17. Reference corpus = Kim's own 167 masters, NOT
