...
- **The clip that was never quiet** — a render can emit a NaN latent that decodes to a full-scale constant: peak 1.0, RMS 1.0, maximum-volume noise. It is not short, not quiet, not truncated, so the file count, the duration probe and the exit code all pass it. Found 110 such cells in a fresh pass, then scanned all 74,765 latents on the board and found 682 more across 15 arms, sitting there since August. The first scan covered only native-length clips and made the fault look native-specific — scan the population you want to make a claim about, not the one your hypothesis points at. Quarantined, and `model_matrix_gen` now refuses to write a non-finite cell — a dropped cell is simply missing, so a resume re-renders it instead of the board serving noise forever.
