...
- **The clip that was never quiet** — a render can emit a NaN latent that decodes to a full-scale DC constant: every sample exactly -1.0 (peak 1.0, RMS 1.0). It is not short, not quiet, not truncated, so the file count, the duration probe and the exit code all pass it. Found 110 such cells in a fresh pass, then scanned all 74,765 latents on the board and found 682 more across 15 arms, sitting there since August. The first scan covered only native-length clips and made the fault look native-specific — scan the population you want to make a claim about, not the one your hypothesis points at. Quarantined, and `model_matrix_gen` now refuses to write a non-finite cell — a dropped cell is simply missing, so a resume re-renders it instead of the board serving noise forever.
- **The two Lion arms** — the fusion-vs-adamw campaign's only unrendered checkpoints, brought onto the board. One of them had never been registered at all, so no renderer could have reached it however much GPU time it was given: unreachable is a different failure from unrendered, and only reading the registry tells them apart.
- **A stale path is not a deletion** — three full-FT ladder arms read as deleted-and-corrupted because their manifest root pointed at a freed NVMe copy; all six checkpoints were intact on the eval drive. `[skip-missing]` looks identical either way, and only looking for the file distinguishes them.
