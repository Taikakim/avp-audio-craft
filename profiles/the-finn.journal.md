...
### decision · augmented-set f0: SCALE, and why the a-priori argument beat every statistic we ran
**DECIDED (F+W, Kim's question, 2026-08-12/13): the AVP augmented variants get their f0 by TRANSFORMING the source, not by re-extracting.** Pitch variants (-2,-1,+1,+2 st, tempo preserved): `f0 *= 2**(st/12)`, values only, time axis and voicing mask untouched. Tempo variants (-10,-5,+5,+10 %, pitch preserved): values untouched, time axis remapped by the **EMPIRICAL length ratio** — never the nominal pct, because `target_bpm = round(bpm*(1±pct))` is **capped at 155**, so the true factor is per-track (augment_tracks.py, bungee).

**OUT-OF-RANGE VALUES: KEEP, DO NOT CLAMP — and MARK them (W, settled 2026-08-13).** Pitch-shifting can push content outside melodia's extraction range (bass 30-350 Hz, other 55-1760), so a scaled value there is true pitch the extractor could never have produced. Keep it: "the extractor could not have produced this" is an EXTRACTOR-consistency objection and we already rejected that axis for this head (below). A 28.5 Hz bass note at -2 st IS 28.5 Hz. Clamping is the actively worse failure — it manufactures a PLATEAU at the bound, a run of frames all reading exactly 30.0 Hz where the music is doing something else, and the head would learn a hard floor that does not exist from the loudest, most confident-looking frames. But EMIT A MARK for out-of-range frames (derivable at scaling time from the bound and the shift, no extra computation): source and augmented targets are not comparable in kind there, and a consumer may want to exclude them. Same reflex as emitting the voicing mask separately instead of folding it into the values.

**THE ARGUMENT THAT SETTLED IT IS A-PRIORI, NOT STATISTICAL, AND THAT IS THE LESSON.** Bungee shifts the WHOLE stem uniformly → every voice moves by the same ratio → the piece is the same music transposed → **the true melody of the pitch+2 variant IS the source melody + 2 semitones, by definition of what a uniform shift does.** Any deviation melodia reports — random or systematic — is a fact about its salience function under bungee's spectral side-effects, not about the music. This survived every n=1 problem that bit us all night, where the statistics did not: W's first lead-baseline (21.2%, one track) collapsed to 6.3% over 7; his bass shift-effect (42.9%, one track) collapsed to 17.7% over 4; each time the single-track number had been doing the work. **When an argument from what the transform DOES is available, prefer it to a measurement of what an estimator SAYS.**

**THE REAL AXIS OF THE CHOICE, worth naming for the next such decision: GROUND-TRUTH-consistency vs EXTRACTOR-consistency.** Scaling matches what is musically true; recomputing matches what our extractor would say about that audio. For a head whose point is that a user asks for "higher", ground truth is the target. That is a DESIGN decision and must be separated from the measurement — we spent an hour measuring before noticing the decision wasn't measurable.
