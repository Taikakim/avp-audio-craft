...
### 2026-08-12 — Reading-sweep 5/5 DONE + Kim's off-lane-criterion correction
Read all 5 F-assigned papers properly (Kim challenged twice "did you read the articles?" — I hadn't; started, then finished): (1) Generalised Signature Method 2006.00873 (30c35cc), (2) SigDiffusions 2406.10354 (8517324), (3) Tymoczko orbifold (ef57a4e), (4) Anatomica 2511.20587 (a5e48d4), (5) Sethares-lineage dissonance curve / Guillet HAL-05124442 (a5e48d4). Each: project-POV note in papers/ + knowledge.md row handed to F (F compiles to dodge the concurrent-edit race).

MID-SWEEP: **Kim-direct criterion correction** (via W): the "off-lane = doesn't process continuous-latent AUDIO" test was WRONG. Kim wants exactly the symbolic music-theory papers now — "we need prediction targets, conditioners, possible new time series." Right test (W's 3-part): (1) computable from a rendered clip (even via mir extractors)? (2) askable by ear as "more X"? (3) time-series-able? Yes to 1+2 = control candidate regardless of the paper's own modality. ~13 of G/F's rows were mis-verdicted under the old test; mine were NOT (my thread is melody-control, always target-hunting) — I re-checked all 4 filed rows, they name targets (path-signature feature, voice-leading distance + consonance-from-center, PH-as-structure). Sethares fit the corrected lens perfectly: sensory dissonance/roughness = a control dim / target / per-frame timeseries, Essentia-computable TODAY.
...
## 2026-09-09 → 09-15 — digest, RECONSTRUCTED from my commits (no journal was kept those days)

*Written 2026-09-24 during the weekly, from `git log --author=CONTINUITY`; the reasoning of those
sessions is gone with their context, so this is WHAT landed, not why-in-depth.*
- 09-09: control lane. Logged FusionOpt's autoscale multiplier and stopped its growth-rate default being
  infinite; per-root contour sidecars; DoRA params into the fusion optimizer; smoke grads sampled at
  backward, not the optimizer step. `--smoke` had been reporting OK on zero evidence: fixed. Latent-scale
  audit: level thresholds can't separate arms, but the within-arm slope can. EXPERIMENTS D17: morph
  conditioner at capacity, four silent-null bugs. FusionOpt hyperball froze zero-init params at zero:
  fixed. LionSR got an optional D-Adaptation estimator.
- 09-10: EXPERIMENTS D18, and a seventh accept-but-ignore flag (plus the general audit); the inference
  UI's render-server half (presets, A/B slots, head metadata, sweep, continuation).
- 09-11: recover online weights from a pair of EMA checkpoints; the morph A/B on KIM-TASKLIST as three
  runnable blocks.
- 09-15: CLAUDE.md note — background tasks die to Claude Code's own memory reaper (MemFree, not OOM).

## 2026-09-21 → 22 — Antigravity's modular optimizer, taken over: most of it was inert

Kim ran an externally-authored modular trainer; it NaN'd and crashed the GPU. What I found, in order:
- **The NaN guard was NaN-blind** (`nan > 1.0` is False → "healthy"). **The loss monitor showed half
  the loss** (Lightning divides by accumulation before callbacks see it). This was the real answer
  to "why is the loss half what I'm used to", superseding my first guess about the corpus.
- **Wrong first diagnosis, retracted:** I proposed rendering clips after training to dodge the NaN.
  Kim pushed back: rendering had worked fine until recently. Then I found the demo callback reseeding
  the GLOBAL RNG per clip. The save/restore fix is still unbuilt.
- **Two regressions of my own:** I widened `_fusion_opt()` to duck-typing (broke `set_loss`, now a
  separate `_sf_opt()`), and threaded `precond_alpha` without declaring it (TypeError). Both caught
  on the next run, not by me.
- **Five or six mechanisms inert across runs:** escape velocity (d pinned at 1.0, costing 0.6 GB per
  checkpoint), VADD tier 2, Schedule-Free with c_warmup ≥ total steps. Built `mechanism_audit.py`
  (static reachability + runtime inertness).
  **My own audit cried wolf twice:** INERT on an unrequested mechanism, and a false negative on VADD
  tier 1, which I'd derived from the wrong statistic. Kim's log showed barrier loss 0.002.
- **Whitening is not viable** on 12288-wide factors: eigh 7.23 s × 24 tensors. The covariance probe
  measured effective rank 90–228 of 8192. The probe's first version sampled only layers 0/1/2
  (a sort tie), which was also the most optimistic sample.
- Read Mousse / KL-Shampoo / DASH. I nearly reported Mousse's double S application as a sign error;
  Algorithm 1 says otherwise.

## 2026-09-22 → 23 — the audition, and the adapter reads a random slice of its input

- `audition_160ep_2026-09-22-b` (subset300, cubic5 + NorMuon + SF, lr 5e-4) was the first config clean
  on every instrument: velocity 57 → 36 per 1k steps, cosine 0.51 → 0.76, crackle only in the 20 s
  rare-prompt clip and gone by step 720. Kim: "very clean, small background sounds better defined, not
  yet 90s goa". **Subset300 is only 42% 1990s** by release year (caveat: reissue dates).
- Post-Grokking paper → the LoRA gauge (B·A = (BX)(X⁻¹A)). Built `lora_gauge_drift.py` (Sylvester
  projection, self-tested). B velocity equals B·A velocity, so the velocity numbers are real; gauge
  share 4.8 → 12.4%.
- **A's row space was 99.2% unchanged** over 1200 steps. The ×20 A-step arm rotated A (overlap
  0.67), but ΔW grew 2.3× and one render went NaN: **confounded**. W's Audiobox later scored it worse
  (CE 6.84 → 5.50). So the "A matters" reading isn't supported; LoRA-TSD separates the two effects.
- LoRA-TSD benchmarked: 8.5 s/step unbatched on ROCm (QR latency 12–17 ms per call).
- run_meta.json had been written to the PARENT dir all along; fixed. Caption routine ported to the
  modular trainer: the goa bigset's sidecar never matched (it would have trained on "None"), and
  `train_lora.py`'s `--source_weights` is inert.
- Mistake: I rewrote `models_index_overrides.json` with a different JSON indent (15k-line diff) and
  caught it before committing.

## 2026-09-23 → 24 — the three-corpus run NaN'd, and the cause was the DoRA magnitudes

- `goa3_avp_r256` (rank 128 despite the name, 3 corpora, lr 6e-4) went NaN at ~step 6900 after 9 healthy
  epochs. **Cause: the small global-conditioning DoRA magnitudes (≈0.13) crossed zero under fixed-size
  sign steps**; the ~2.4 transformer ones didn't. A 48 s demo render went NaN ~600 steps first. Best
  checkpoint: 5072. My `run_meta` recorded the run as "done" (bug, fixed).
- Gemini deep research (two replies): the first was a reconstruction. The second verified Muown,
  LionVote and M+Adam; it misread DoRAN as being about the magnitude. Built
  `--modular-magnitude-update multiplicative` (7 tests). The retry is pending cheap electricity.
- Kim's asks: an operator manual for every flag (`docs/train_lora_modular.md`, kept honest by
  `check_train_manual.py`), all training-failure causes/effects in `docs/training-findings.md`, read
  papers filed with reading-depth sidecars. Pushed all three repos at Kim's request, attributing other
  instances' work to them.
- LoRA-TSD batched port under way: 4 parallel workers + an Opus critic. Worker A validated the
  reference against upstream (8/8), batched ≡ reference (24/24), and found a state_dict aliasing bug in
  both (A fixed its own).
