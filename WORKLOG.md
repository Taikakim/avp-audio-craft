...
- [2026-08-18] (the-finn) ARCHITECTURE.md + CLAUDE.md: added the missing location-oriented index (Kim
  direct — "agents are quite clueless about existing data locations when there's so many"). New §"Where
  things live": a 33-row live-swept table of every SAO/ top-level folder (purpose + pointer into the
  existing §A–F tool-reuse index), the UUID drive's actual contents (`sao_models/`, `goa_archive_features/`,
  `suomisoundi_*` — none of this was documented anywhere before), and new LUMI `/project` vs `/scratch`
  path breakdowns. Flagged, not resolved: `goa_archive_captions/` exists as two different directories with
  the same name (top-level UUID drive vs nested under `lumi_runs/`). CLAUDE.md now marks ARCHITECTURE.md
  required reading *before planning*, not just before coding; retired the stale inline `## Layout` list
  that had drifted out of sync (pointed it at the new map instead of duplicating it a second time).
  Coordinated disk cleanup same session with WINTERMUTE (his 117G models-move + my 12.9G of dead venvs/
  ONNX-export artifacts) — SAO/ 276G→112G, nothing deleted that either of us hadn't independently verified.
- [2026-08-18] (the-finn) Fixed a broken LUMI sanity16 job chain directly via ssh (CONTINUITY's DM, Kim
  unreachable for several days). Root cause: a `SMOKE=1` flag dropped from a pasted launch command, three
  times running, turned a 20-step smoke gate into a full 60-epoch run capped at a 40-minute timeout — so
  `--dependency=afterok` never satisfied and the whole stock-Stability sanity-control matrix Kim asked for
  before leaving silently never ran. Verified live `squeue` state myself before each action rather than
  act on DM narration (the described state had drifted twice between messages). Rsynced CONTINUITY's
  script fix (which also closed a worse defect: the script's bare `wait` reported success even if every
  arm crashed, making the gate meaningless in both directions), cancelled the broken chain, submitted
  directly. Re-checked 08-21: 2 of the 3 jobs I'd submitted had actually failed (`torchcodec` ImportError
  at the periodic demo-audio-save step, not the training loop) and a resubmit hit a third distinct bug
  (a caption-key mismatch on `latents_goa_bigset`'s synthetic ids vs a relpath-keyed sidecar — the same
  failure class CONTINUITY closed separately that night on the `winning_fleet`/`fullft_fleet` campaign).
...
- **2026-08-24 (C)** — Inference-UI planning complete: all four plans now under `docs/superpowers/plans/` (model-DB+A/B slots, global conditioner+inpaint, pianoroll+contour, batch-sweep+head-metadata). Diagnosed the empty UI pickers — NOT a backend fault: three narrow discovery paths (`controls.py:41` hardcoded DoRA list, `explorer_render_server.py:83/93/306`) leave ~362 of ~2106 model files reachable; all 833 control adapters in `sa3_control_runs` are `.pt` and invisible by extension. Measured the 16 LatCH head checkpoints (`std_mean/std_std/loss_type/epoch`) — the range meter Kim asked for needs no dataset pass, and the shared `slider_min -80/max 20` is wrong for most heads. Audited `/home/kim/Projects/steer-sao` (sibling MuseControlLite impl): lift multi-branch control-CFG + dynamic latent length, `control/RESEARCH_RADAR.md` §7. Corrected three docs (mir venv is `mir/mir/bin/python`, latent player runs under `stable-audio-3/.venv`) and the LUMI purge date (data removal ≈2026-11-20, not late Aug).
- **2026-08-26 (GHOST-NOTE)** — Longform seam repair (Kim's "bursts" complaint on the 570s
  `fp32cmp_avp_t4096_bs1_lr1e4` ep7 continuation): root cause is each sliding window being an
  independent fresh-seed sample softly conditioned on the prior tail, so energy/dynamics reset
  per-window even though style stays consistent. RMS-guided continuation (LatCH constant head)
  made it WORSE (negative result). Post-hoc SDEdit reanchor at seams improved DSP metrics but
  failed Kim's ear test. **Winner: true bidirectional native inpainting of the transition gap
  itself, wide symmetric context (~20s/20s) beats asymmetric or narrow context.** Landed
  `LongFormRenderer.render_latents(init_latents=...)` (stable-audio-3 `43037de`) and `/longform`
  `init_latent_path` (SAO `2e3c8dd`), coordinated with CONTINUITY who owns
  `explorer_render_server.py`. Full detail + the beat-snap break-detector build:
  `profiles/ghost-note.journal.md` 2026-08-25/26.
  Also root-caused `suomift_goaft`/`suomift_avpaug19` ("identical every epoch" / "pure spectral
  noise every epoch"): `train_lora.py`'s SimpleEMA defaults (`--ema-beta 0.9999`,
  `--ema-warmup-steps 100`) give a ~10k-step time-constant that a short warm-start run (likely
  <1000 total steps) never moves off init — the "noise" is the un-fine-tuned bare-backbone
  starting point, confirmed by diffing against already-scored `bb_avpaug19`/`bb_goaft` renders.
  Proposed fix (explicit `epochs × dataset_size / effective_batch` beta scaling, not buried in an
  opaque `total_steps`) is validated with Kim but **NOT YET IMPLEMENTED** — anyone relying on EMA
  checkpoints from a short warm-start fine-tune should know the shadow is likely still at init.
  evaluator.html UX pass (single question, persistent help modal, silent PQ<3.5 floor in
  `build_evaluator_manifest.py`) shipped, SAO `36dc484`.
- **2026-08-26 (C)** — **Inference UI: the LatCH panel stops lying** (Kim's 2026-08-24 ask, plan `docs/superpowers/plans/2026-08-24-inference-ui-batch-sweep-and-head-metadata.md` Tasks 1-3). New `eval/head_meta.py` derives **per-head slider bounds from each checkpoint's own dataset statistics** instead of one shared -80/20/-30; the panel now shows a dataset range meter, disables the value/kind/loss controls on heads that cannot use them (384-ch `same_chroma`, 12-ch `hpcp`, cosine objectives), badges undertrained heads (`spectral_kurtosis`: 3 epochs, σ 521.6) and carries hover help on ρ/μ/γ/n_iter phrased in the server's NORMALISED scale. Also fixed the documented KNOWN BUG: pickers filled from `Input(<id>,"id")` fired once per page load, so with :8056 down at load they stayed empty forever — a shared 5 s `dcc.Interval` now retries and the pickers refill **without a page reload** (verified: server killed, viewer restarted, server restarted, options went 0 → 16 in the same process). Trainer now records `target_source/chroma_dir/chroma_key/db_path` so future heads are traceable. 51 tests green (17 SAO + 34 mir). NOT committed.
- **2026-08-26 (C, cont.)** — **Inference-UI roadmap: presets, root selector, sweep surface, continuation provenance.** `eval/presets.py` + `GET/POST /presets` + a Preset row in the inference tab (a preset is the `/generate` payload under a name minus seed/batch, PLUS a viewer-form snapshot, because `build_payload()` is not losslessly invertible; measured round-trip 63/63 fields, rebuilt payload identical to the stored one). Root selector + model-info panel over `/roots` and `/models` — the ckpt picker is no longer pinned to one root, labels carry family/corpus/rank/epoch, and picking one shows recipe + why + verdict + provenance (gap #1 of seven in INFERENCE-SURFACE closed). `eval/sweep_spec.py` + `sweep_run.py` + `build_sweep_page.py` = the batch/sweep surface (gap #4), a thin client of :8056 with a resumable fsynced manifest; proven end-to-end with real renders. `eval/continuation.py` — `/longform` recovers the prefix's own ckpt/dora from its sidecar so a tail cannot silently be a different model than its head, and the t2a path finally saves z0. **Two findings for Kim:** `/generate` and `/a2a_*` still save NO z0 (the latents never surface from `MODEL.generate`; the clean fix is inside the fork, left for him), and importing `Misc/build_evals.py` poisons `sys.path` process-wide (its line 21 prepends `Misc/`, where our `filelock.py` shadows the pip package) — load it by path and restore. 128 tests green. NOT committed.
- **2026-08-26 (C, cont. 2)** — **z0 sidecars (Kim approved the fork edit) + resident A/B slots.** `latents_sink` in `model.py:generate()` / `sampling.py:sample_diffusion()` captures the latents without touching the audio path — **byte-identical on BOTH sampling branches under a same-seed A/B** — so `/generate` now writes `out_NN.z0.npy` and every response carries a top-level `latents` list. Proved end to end: a 10 s `/generate` continued through `/longform init_latent_path` came back 24 s with **prefix correlation 1.000**. The a2a paths save none on purpose (their output is a crossfade of separately sampled windows, so no single z0 made it) and say so in `meta.z0_reason`. Then `eval/adapter_slots.py` + `/slots` + `POST /ab` + a viewer panel: N adapters resident on one base, `rebuilt=False` on every switch, slot 0 / slot 1 / base all distinct and returning to slot 0 byte-identical. **Two measured corrections to the plan's VRAM estimates:** an r128 DoRA costs 0.40 GB not 0.33 (`model_db._GB_PER_RANK_HALF` fixed), and medium-base leaves 7.40 GB free, so the baseline is ~8.5 GB not ~5.1. **One bug caught by testing the control arm:** a `slot: null` render came back byte-identical to the previous slot-0 render — "base model" was quietly still the last adapter, because ACTIVE_SLOT kept its value. Fixed and pinned by a test. Model census rebuilt post-remount (363 arms, 202 on LUMI, 89 LUMI-only, 132 with no fat copy anywhere). 163 tests green. NOT committed.
- **2026-08-26 (C)** — **Pianoroll/contour Task 0 PROBE: all four steps pass, the plan is GO.** 32 `melody_contour` checkpoints on the UUID drive (headb_melody vocab **9**, morph L2=5, IOI3/L3=15, L4=77) — found by reading each zip's `data.pkl` member instead of `torch.load`, 865 files in minutes. **The pianoroll arm is ALREADY PULLED** (plan assumed it was still on scratch): `pianoroll_fullft/proll_fullft_t256_bf16_s{1,2}`, terminal ckpt 12.9 GB FAT each, with `control_ablation.jsonl` showing control gain vs shuffled settling at +0.0068/+0.0112 across two seeds, monotone after step 800 — the DiT reads the roll, though that is a training-loss signal, not audible control. The modular inlet installs (12 of 24 blocks, 30.7 M params) but **the plan's accessor is wrong by one level** — use `m.model.model.model.transformer`, not `m.model.model`. `contour_streams` imports cleanly, no vendoring. **Two corrections recorded for the implementer:** `headb_melody` does not store `melody_vocab` in args, so the loader must fall back to the embedding shape or it will reject the plan's primary target; and a cloned **torchcodec source repo at `SAO/torchcodec` shadows the real package** whenever the SAO root is the cwd, breaking `T5GemmaEncoderModel` and every `from_pretrained` for `python -c`/heredoc runs but not for scripts (MASTER §5). Findings in `docs/INFERENCE-SURFACE.md` §9. NOT committed.
- **2026-08-27 (C)** — **LatCH guidance budget now normalised over the ACTIVE WINDOW (BEHAVIOUR CHANGE, Kim-approved).** Review prompted by Kim after the Pons LatCH paper: `_st_weights` divided by the sum of alpha over ALL steps, so a windowed guide spent only its window's share of a whole-schedule budget. Because alpha = 1-sigma is tiny at high noise the loss was severe and invisible — measured on a real 24-step schedule the **first 20% of steps carried 0.2%** of the budget, the GUI default 0.0-0.6 carried **9.3%**, the last 20% ~62%. So the paper's own 'guide the first 20%' was not expressible here, and an A/B of it would have returned a null I'd have misread as 'selective TFG doesn't port to RF'. Each guide now normalises over its OWN window (per-guide `_scale` folded into the loss — grad is linear, so identical to scaling the gradient), in BOTH samplers. **⚠ Legacy gains are now ~10.8x too strong at the default window — divide by ~10.** Verified on GPU: a 0.0-0.2 window went from delivering ~nothing to bass-fraction 0.29 (vs 0.11 at 0.0-0.6) and crest 2.21 vs 6.29 — potent, and at gain 512 probably too strong, which is exactly the paper's rho/mu caution. Also: corrected the gamma help (gamma is input-NOISE std, HIGHER = less precise = SAFER; I had it inverted), n_iter's help (strength knob, not only cost), and `papers/knowledge.md`'s claim that our LatCH differs from the paper's — it does not. 133 SAO + 48 mir tests green. NOT committed.
- **2026-09-02 (C)** — **`docs/GIT-PROTOCOL.md`: the fleet's git manual** (Kim's ask, after the multi-repo commit campaign). Compiles what the campaign cost us into one operational doc: the **repo/remote map** (stable-audio-3 and stable-audio-tools are NESTED, mir and fusion-optimiser are SIBLINGS — `git -C fusion-optimiser` from the SAO root fails with "cannot change to", which reads as "repo gone"; push targets are `origin` everywhere except **stable-audio-3 → `fork`**, whose **`upstream` is `Stability-AI/stable-audio-3` WITH a push url**); **identity** — commit via `Misc/agent_commit.sh <HANDLE>`, and the honest note that the convention was adopted in `20b4852` and the very next nine commits (mine) still landed authored "Kim", because it fails silently and `--oneline` hides the author field; the **backtick incident** — DM prose containing `` `git push` `` passed inside a DOUBLE-quoted Bash string was command-substituted and published 180 unapproved commits, hence *never interpolate prose into a double-quoted shell string, build it in Python and pass argv or use `-F <file>`*; the **never-commit list**; the **torch-pass protocol** for multi-author dirty trees plus the `git add -p`-free hunk/sub-hunk splitting recipes and staged-blob verification; and a one-page checklist. Also: **never narrow the output of a command you are about to make a claim from** — `git remote -v | head -4` is how I told a teammate stable-audio-3 had no `upstream` (the lines were 5 and 6). Registered in `ARCHITECTURE.md` doc map, a 4-rule summary in `CLAUDE.md`, push targets in `MASTER.md` §1; the splitting recipe moved out of `docs/lessons-learned.md` (which keeps the three-leg attribution ANALYSIS) so the two cannot drift. NOT committed.
- **2026-09-02 (C)** — **Census rebuilt end-to-end + run-purpose recovery (Kim's ask).** LUMI side regenerated over one multiplexed ssh; **caught my own find pattern being too narrow** (`-name '*.ckpt'` drops the **528 `.pt`** heads/riffers = 85 arms incl. all of `ftstack_heads`, and looks exactly like deletion on LUMI). Also classified the **2026-08-28 prune** (`prune_dirs_2wk.txt`, 86 dirs, fat→slim): all 1073 changed paths have a `.weights.ckpt` in place, **0 genuinely lost**. Census now **369 arms**, local 348, **LUMI-only 89→21** (the 67 GB pull landed; 22/22 verified size-matched to LUMI, all open, all FAT), no-fat-anywhere 132→122. **Purpose 90/363 → 205/369**; of 226 locally-resumable arms, 198 have one. Method (F's pilot + Kim's chain): **Tier 1 = sbatch header, ~free** — new `eval/mine_purposes.py` joins arms→trainer and collapsed 126 arms to **23 distinct headers**, so it was 23 judgement calls not 126; **Tier 2 = transcript archaeology**, F, ~1-3 greps/cold arm, ~1 warm. **THREE INSTRUMENT BUGS, all 'the join was right, the meter was wrong'**: (a) the first join gave 44 `dorlor_ab` arms to `a11_full_render.sbatch` — a RENDER script NAMES every run it renders, so content-matching cannot see ROLE; fixed by ranking scripts that invoke `train_lora` first (126/126 now real trainers). (b) my purpose keys used the census slash-path; `model_db` keys on the **leaf dir** — 126 writes silently missed, and 11 arms end in a literal `checkpoints/` dir whose leaf would have applied ONE purpose to 11 unrelated runs (now fall through to `family_dir`; 9 `_mt` twins merged with an explicit ambiguity warning). (c) **the real one: `model_db.load_or_build` baked the overrides join into its cache**, so a freshly written purpose could not appear until a ~50-min rescan — which is why fixes (a) and (b) both 'worked' and changed nothing. Fixed: `reapply_curation()` re-joins the cheap hand-edited layer on every load (cache is for expensive PROBE data, not for metadata edited hourly). 12 tests green. Also `--rescan` flag on the census builder (it hardcoded `rescan=False`, silently reproducing a 26-08 cache). NOT committed.
- **2026-09-03 (C)** — **Morph-conditioner audition site built; two melody lanes pulled off LUMI.** Kim asked to render clips + build an eval site: **the clips already existed** — the morphcond grid (384 cells, 16 arms × 8 stems × {off, g1.0, g2.0}, cfg7, z0 + contour stream saved per cell) had been rendered weeks ago on the UUID drive and never auditioned, and the census said `clips=-` only because those renders were never linked into `model_matrix.html`. **My earlier "never rendered" report was wrong; Kim's "I thought we had at least the morphcond flats" was right** — checking the drives beat trusting the census, again. So: transcoded 392 clips to the serving codec (AAC 192k, matched to headb_bracket) and built **`eval/build_morph_page.py`** → `~/evals_aac/morph_conditioner/` — one same-playhead table per stem, each with its REFERENCE clip and the control-off column (the projection is zero-init, so off = the plain model = a true control). 392 buttons, 0 broken links. Reuses build_evals PLAYER_JS, loaded BY PATH. **Also pulling `ftstack_heads` (B9: Head-B on the full-FT backbone + 6 LatCH heads) and `morph_head_sweep` — 220 files / 86 GiB that were LUMI-ONLY with the purge ahead**, smallest-first with an 8 GiB disk guard (`Misc/pull_heads.sh`). **Two registry corrections:** B7 (MIR-timeseries conditioner) is marked RUNNING since 08-21 but has **ZERO output anywhere** — 0 mirctrl files on LUMI, 0 in the census; and **D16's premise quoted D15's RETRACTED `+0.022 n.s.`** when D15's revised verdict is `+0.100, p=0.016` — D16 re-scoped to "how fast a trajectory", not "does timing transfer". NOT committed.
- **2026-09-04 (C)** — **Morph-conditioner measurement: FOUR instruments, four failed gates, no verdict — plus two new brackets built.** Kim asked for melody similarity on the morph grid. **MERIT** (AMAAI-Lab, already cloned at `~/Projects/MERIT`, wrapper `control/sa3_control/merit_eval.py` — Kim caught me about to re-invent it by transcription): self-check exact 1.000, but **S_mel is SATURATED on same-corpus material** — two DIFFERENT references score 0.611 mean/0.861 max while a cell scores 0.49-0.69 against its OWN. Decisive test: at gain 2.0 conditioning raised FOREIGN similarity MORE (+0.041) than own (+0.025), diff -0.017, p=0.71 ⇒ **the +0.025 I nearly reported as "the conditioner works" is a generic style shift, not reference-specific.** **D15's transcription measure** then failed its own dynamic-range gate here (own-vs-foreign melody +0.022, 80/192, p=1). **Both failed for one STRUCTURAL reason**: `contour_codes.py` — *"Monotone invariance is the point: dense rank is unchanged by any strictly increasing transform"* — the contour stream encodes RANK ORDER only and discards absolute pitch by design, so MERT melody embeddings, pitch-class cosine, pitch-set Jaccard and note-cell F1 all measure exactly what the conditioner throws away. A perfectly-adhering render scores at chance on all of them. Built the matched **contour-to-contour** test (`eval/morph_contour_ab.py`) — it FAILS ITS OWN POSITIVE CONTROL: re-extracting from the REFERENCE AUDIO, the exact source the fed stream came from, scores 0.146, LOWER than the renders (0.151). Cause: fed stream = `f0_other_ts` (stem-separated, continuous, pre-encode) vs my MuScriptor skyline (polyphonic transcription of DECODED audio). Control is now wired in as a FATAL gate. **Encoder control against ground-truth `f0_other_ts` reaches only 0.755/0.550/0.337 for L2/L3/L4** — degrading with alphabet size = a grid/window OFF-BY-SOMETHING, not a wrong feature; fixing that is step 1 next session and it unlocks the whole test. ⚠ **Without the controls I would have reported a false result THREE times.** **What stands:** gain 1.0 does nothing measurable on any axis; gain 2.0 changes audio non-specifically; on the Pattern-2 full-FT backbone conditioning degrades 60/64 cells with S_tim collapsing 0.373→0.169 (matches Kim's ear). **NEW TOOLS:** `eval/soup_pt_ladder.py` (blend post-trained `medium` toward `medium-base`; measured first — 997 identical tensors, **median relative delta 0.0013, median cosine 1.0000, 6/899 below cos 0.9**, change CONCENTRATED in `to_local_embed` biases of layers 16-19 ⇒ linear interpolation is in the safe regime and a TARGETED rewind is possible; both endpoints verified exact) and **`--spectral-lr`/`--scalar-lr` on train_lora** — `build_fusion_param_groups` always accepted them and NOTHING ever passed them, so every Fusion run we have ever done used ONE lr for both groups, which is a candidate explanation for our 5e-6 sitting ~200x below Zach's Muon rule of thumb. Omitting both threads no key ⇒ byte-identical to all prior runs (verified). Also: `ckpt_tag()` collapsed `-vN` replicas onto one label, destroying replica identity in the render filename upstream of all 22,036 ratings — fixed forward-only, 4 tests. NOT committed.

## 2026-09-08 — GHOST-NOTE — NaN latents decode to a full-scale DC constant; 165 cells quarantined, guard added
Rendered the last week's unrendered autoscale arms (`lion_lr1e-5`, `lion_lr5e-5-batch32` — the
latter was never registered in `eval/rarity_bracket_manifest.json`, so no renderer could reach it —
plus the `fullft_ladder_{A,B,C}` ep19 midpoint, whose manifest root pointed at a freed NVMe path
and therefore read as deleted when all six checkpoints were intact on Mantu).
**Found a render fault that predates today:** a NaN latent decodes to a FULL-SCALE CONSTANT
(peak 1.0 / RMS 1.0), which passes file-count, duration and exit-code verification alike. 110 such
cells in the fresh pass; scanning the corpus found **682 more** — 55 of 7,989 native latents AND
**627 of 67,459 standard 20 s latents across 15 arms**, so it is not native-only and has been on the
board since at least 2026-08-04. All quarantined
(`Mantu/sa3_lora_runs/model_matrix_QUARANTINE_2026-09-08_nan/`), manifest lines stripped, stale
`clip_metrics.db` rows deleted; board re-verified at **74,765 latents, 0 non-finite**. None of the
682 was ever rated and none is in the evaluator pool. The 15 arms now have GAPS where the noise was. Cause is the long-known broken-model family, not a new render bug (per project guidance the same
day): see `DISCOVERIES.md` 2026-08-10 full-FT latent-scale runaway, C's cautious-rescale norm
inflation that NaN'd `sa3-goa-dora-47s-r128-fusion-caut` (108 of the quarantined cells), and the
2026-08-11 `_QUARANTINE_fullft_bigset_latent_runaway`. One arm (`lion_lr1e-5`) does NOT show the
weight-growth signature — global L2 2327 vs clean siblings 2345/2322 — and that anomaly is recorded
in MASTER §5 rather than resolved; parked for budget.
Guard: `model_matrix_gen::z0_is_finite` (commit `dd0300f`, 9 tests) refuses to write a non-finite
cell, so a dropped cell is simply missing and a resume re-renders it. Details + the untested lead:
MASTER §5, `profiles/ghost-note.journal.md` 2026-09-08.
**Also:** Audiobox scoring was silently dead again (0 scored, exit 0) — `libbluray` .so.3 -> .so.4
...
- [2026-09-09 07:55] (WINTERMUTE) LATENT-SCALE HEALTH: three negative results and one surviving meter. Thread closed with CONTINUITY (tool 573c0b2); recording so none of it becomes folklore.

CONTEXT: the publish gate's latent-sanity audit (model_matrix/.latent_sanity.json, 13282 operating-point clips) reported 285 over Z0_STD_MAX=2.0 and ZERO non-finite -- the NaN quarantine and z0_is_finite guard held completely.

NEGATIVE 1 -- no LEVEL-based threshold on latent scale works, per-family included. base_ptm__base, the unmodified post-trained model and healthy by definition, has median global std 3.47 and 250/256 channels over 2.0: the second-highest-variance arm in the corpus. The healthy population is heterogeneous BELOW the family level, and a per-family baseline is built from adapters that pull toward the middle, so it is LESS extreme than the raw model it must normalise. Per-family standardisation is the same defect one level down, not a weaker version of the fix.
NEGATIVE 2 -- per-CHANNEL counts do not rescue it. n=3423: ptm-flagged vs non-ptm bias is 4.84x on global std, 5.92x on any-channel-over-2.0 (WORSE), 4.87x on channels-over-6-robust-sigma of their own family baseline (no better). Max channel std does not discriminate at all (<2x); it was never the worst channel that carried signal, and now not the count either.
NEGATIVE 3 -- an arm-vs-its-own-parent ratio, the obvious fix for single-checkpoint arms, INVERTS the bias: over 1.5x fires on 1.0% of ptm arms vs 9.1% of non-ptm. Cause is worth keeping on its own: adapting the post-trained model systematically REDUCES latent variance, ptm adapter arms rendering at ~0.46x their nominal parent across 209 arms. So "ptm" is not one population with an outlier, it is the raw PT model AND a tighter adapter population sharing a sampler.

WHAT SURVIVES: an arm measured against ITSELF across epochs (peak/first median z0 std). 99 runs with >=3 epochs: p50 1.20x, p90 2.74x, p99 5.87x, max 52.58x; 18/99 over 2x, 6/99 over 3x. No cross-arm calibration, no family term. It VALIDATES by recovering the lreq ladder's lr ordering unprompted -- lr2e4 52.58x, lr1e4 4.26x, lr5e5 3.42x -- i.e. it reproduces the runaway mechanism (higher lr -> weight growth -> latent scale) from a statistic told nothing about lr. Reads the collapse epoch with no render and no ear: adamw_goa_t512_bs1_lr1e4 ep5 median 1.29 -> ep9 2.67 (max 163.98); fullft_bigset ep3 1.06 -> ep7 4.19.
COVERAGE GAP, stated in its real size: of 879 arms, 99 have >=3 sampled epochs and are covered, 159 have exactly one, 157 carry no epoch suffix. The trajectory statistic monitors a minority of the corpus and single-checkpoint arms have no within-lineage reference at all. Unclosed, deliberately, rather than shipping the inverting rule.
DISPOSITION: Z0_STD_MAX stays as a crude non-finite-adjacent backstop; per-epoch slope goes in as TRAINING telemetry, not a publish gate. Gate untouched.
...
- [2026-09-25 08:31] (CONTINUITY) 2026-09-25 CONTINUITY: (1) LoRA-TSD batched optimizer committed (SAT fdff0ea, SA3 ea829f0; --optimizer lora_tsd, manual 5e), GPU speed unmeasured. (2) New default-on diagnostic in train_lora_modular: --flight-recorder logs raw pre-clip grad norms grad/raw_norm_* and dumps spike incidents to <run>/incidents/ (read: eval/inspect_flight_incident.py). (3) Step-governor proposal reviewed, not adopted (update norm is blind to bad batches under normalised optimizers): docs/PROPOSAL_ADAPTIVE_P95_STEP_GOVERNOR.md.
