<!-- Static legacy snapshot for build_model_index_page.py. Hand-authored 2026-07-04 by-ear
     verdicts + off-board control-head notes (onset FiLM / style fingerprint / LatCH / ES /
     mutated bases / soups). These artifacts are NOT on the live text-to-audio eval board
     (manifest_live.jsonl), so the generator cannot reach them -- kept here verbatim so the
     by-ear verdicts survive regeneration. Edit freely; the generator appends this file as-is. -->
