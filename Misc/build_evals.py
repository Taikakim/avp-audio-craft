...
def desc_for(name):
    """best description for a run/set folder from run_purposes.json, else a parsed hint."""
    for k, v in PURP.items():
        ...
        if v.get("run") == name or k == name or (v.get("run","") and v["run"] in name):
            ...
            return (bits[0] or "").strip(), extra
    # parsed fallback from the dir name
    return "", name.replace("_"," ")

def clean_label(fn):
    return re.sub(r"\.m4a$","",fn).replace("__"," · ").replace("_"," ")
