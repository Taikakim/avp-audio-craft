...
def build_landing(control, renders):
    ...
    _highlights = [
        ...
        ("/evaluator/?goa=1", "Blind A/B + enjoyment rating — phone-first",
         "Same-playhead A/B/EVEN comparison across the quality questions (high-frequency "
         "detail, soundstage, structure on native lengths only, interesting) — must answer "
         "all of them before the next pair loads, so every pair yields complete data from "
         "the start. Plus a 0-5 enjoyment score per clip (0 = technically broken, kept out "
         "of any mean/median). One-thumb, built for listening on the go."),
