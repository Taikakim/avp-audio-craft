...
| `--no-inline-demos` | off | With `--eval_demos`: milestones still save `step=N.ckpt` and the loss guard stays on, but **no clips render inside the training process**. Use it on DoRA runs: a live adapter rendered between training steps returns random NaN/1e11 latents on our ROCm stack (training-findings 13e). Render afterwards, exactly, with `stable-audio-3/scripts/demo_cfg_sweep.py --run-dir <run> --steps-ckpt <N…> --cfgs 7 --write-demos` (merges the adapter; `--write-demos` also puts the clips in `<run>/demos/step<N>/` under the usual names). |
