...
- **Layer-localization story (#44) being rewritten** (C, 2026-07-21): mid-stack L8–15 is load-bearing (L14 alone ≈ full 24-tap adapter; L12/TADA-site inverts, explaining the old "mid-group inverted" 07-12 reading; "write-site = compute-site"). The **paper-verdicts TADA entry** (public) currently frames it as "acoustic attributes localize to late 16–23, opposite TADA's 12–13" — **verify + update that entry once C's rerun settles into a verdict** (feeds task #53 layer-restricted-adapter arm). The W28 chronicle synopsis correctly records the 07-12 state and stays as-is (point-in-time). Owner of the finding: C; F reconciles the doc when stable.
