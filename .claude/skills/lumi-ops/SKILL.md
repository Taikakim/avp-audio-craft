...
| 1b | **are you searching the right NAME?** | `grep -n 'job-name' <the sbatch>` then `sacct -X --name=<that> -S <date>` | **`sacct --name` matches the SLURM `--job-name`, NOT the run label.** A fleet sbatch is typically `--job-name=winning_fleet` while each arm carries a distinct `--name` passed to `train_lora.py` (`wfleet_suomi_t1024_a45_fp32_s1`) — SLURM never sees that string, so querying it returns EMPTY for a job that ran fine. Also: **`sacct` defaults to jobs started TODAY**; without `-S <date>` anything older is invisible. An empty `sacct --name=<run label>` proves nothing on either count. Same family as the `squeue` trap in row 1 — the query answered a different question than the one asked. **Settle it on disk instead:** `ls <RUNROOT>/<arm>/epoch=*.ckpt` — the artifact cannot lie about whether the arm exists. |
