...
// per-clip bad-sample count (Kim direct 2026-09-24: "a column with per-clip bad sample
// count... per selected clip") -- basename-keyed, built by build_bad_samples_index.py from
// clip_metrics.db's n_bad_jumps (see audio_corruption_scan.py's calibration note: a small
// count often blends in inaudibly, it's a screen not a verdict -- shown as a plain number,
// not a pass/fail badge, for exactly that reason). Best-effort: absent on file:// / offline.
let badIndex=null;
function badLabel(file){
 if(!badIndex||!file)return'';
 const n=badIndex[file.split('/').pop()];
 if(n==null)return'';
 return n>=999999?' · NON-FINITE LATENT':(n>0?' · '+n+' bad sample'+(n===1?'':'s'):' · 0 bad samples');}
...
function repickCurrent(){
 ...
 playingKey=key;
 curLabel=hit.mkey+' '+playingCkpt+' × '+hit.pid+' cfg'+hit.cfg+' w'+hit.w+(hit.exact?'':' ·nearest')+sw.tag+badLabel(sw.file);
...
function playRow(tr){
 ...
 curLabel=hit.mkey+' '+tr.dataset.ckpt+' × '+hit.pid+' cfg'+hit.cfg+' w'+hit.w+(hit.exact?'':' ·nearest')+sw.tag+badLabel(sw.file);
...
function fillPicker(promptTxt,cfgSet,wSet){
 ...
 markAvailability();}
async function loadBadIndex(){
 try{const r=await fetch('model_matrix/n_bad_by_file.json',{cache:'no-store'});
  if(r.ok)badIndex=await r.json();}catch(err){/* offline / file:// -- per-clip count just won't show */}}
async function loadManifest(){
 loadBadIndex();  // fire-and-forget, independent of the manifest fetch below
