...
"""
# --- CPU thread caps + flash-attn off MUST precede torch import -------------------
import os
...
def load_model(ckpt: Path | None):
    ...
    t0 = time.time()
    model = StableAudioModel.from_pretrained("medium-base", device="cpu", model_half=False)
...
def render(model, out_dir: Path, tag: str, steps: int, duration: float,
    ...
    for i, prompt in enumerate(PROMPTS):
        ...
        print(f"[clip] p{i} {dt:6.1f}s  finite={finite} peak={peak:.3f}  "
              f"prompt={prompt!r} -> {out_path}", flush=True)
        if not finite:
            print(f"[WARN] p{i} produced non-finite samples!", flush=True)
    if times:
...
def main():
    ...
    print(f"[cpu] torch threads={torch.get_num_threads()}  "
          f"steps={args.steps} duration={args.duration}s seed={args.seed} cfg={args.cfg_scale}",
          flush=True)

    if args.base:
        model = load_model(None)
        render(model, args.out_dir, "base", args.steps, args.duration, args.seed, args.cfg_scale)
        return

    if args.ckpt:
        model = load_model(args.ckpt)
        render(model, args.out_dir, ckpt_tag(args.ckpt), args.steps, args.duration,
               args.seed, args.cfg_scale)
        return
    ...
    while True:
        ...
        for c in cks:
            tag = ckpt_tag(c)
            done = all((args.out_dir / f"{tag}__p{i}_seed{args.seed}.wav").exists()
        ...
        for c in todo:
            tag = ckpt_tag(c)
            print(f"\n=== {tag} ({c}) ===", flush=True)
            model = load_model(c)            # fresh base each time -> isolated adapter
            render(model, args.out_dir, tag, args.steps, args.duration, args.seed, args.cfg_scale)
            del model
            seen.add(tag)
        if not args.watch:
            return
