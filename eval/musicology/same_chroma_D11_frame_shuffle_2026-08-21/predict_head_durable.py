...
def main():
    ...
    # D11: the tier-2 primary sample first, so the shuffle scorer can start ASAP;
    # the rest of the intersection follows in the same run (skip-done = resumable).
    pri_f = Path(__file__).parent / "tier2_sample_ids.txt"
    if pri_f.exists():
        pri = set(pri_f.read_text().split())
        todo.sort(key=lambda f: (f not in pri, f))
        print(f"[predict] prioritizing {sum(1 for f in todo if f in pri)} tier-2 sample ids", flush=True)
    if args.limit:
