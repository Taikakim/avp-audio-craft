...
2b. **`docs/data.md`** (Kim direct 2026-09-16) — **the training-data census: every corpus,
   target store and database, and what each one can actually TRAIN.** Read it before planning
   any training run or asking where a dataset lives. It is GENERATED by
   `Misc/training_data_census.py` — re-run that rather than trusting or hand-editing the
   counts. It exists because agents kept re-deriving this by `find`/`ls` every session, and
   because two naming traps repeatedly produce confident wrong answers: **`.npy` is always the
   latent and `.npz` is always a target/companion** (so a store can be 100% `.npz` and train
   nothing on its own), and **several TARGET stores are named `latents_*`**. Also holds the
   access snippets for every store and both SQLite databases.
