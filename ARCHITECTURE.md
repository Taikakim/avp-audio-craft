...
| **`eval/soup_pt_ladder.py`** | **dial on "how much post-training"** — `W = base + a*(PT-base)` between `stable-audio-3-medium` (PT) and `-medium-base`, with `--targeted-alpha/--targeted-regex` to hold a subset at a different alpha. Kim 2026-09-04 (PT is punchy+coherent but "always sounds more or less the same with the kick, bass and percussions"). Safe because PT is a FINE-TUNE of base (one trajectory = linear-mode-connectivity regime), unlike the independently-trained soups of C1/C2: measured change is CONCENTRATED not small (median rel delta 0.0013 but mean 0.0215, max 0.714, in `to_local_embed` biases L16-19 — G measured the functional tail as hf_ratio +57% on rendered clips). Endpoints self-verify exactly. 8.6 GB/blend — build, render, delete. |
