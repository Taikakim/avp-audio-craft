...
- **Mantu** (`/run/media/kim/Mantu`) — source audio + checkpoints + the latents backup.
  `ai-music/Goa_Separated` (4470 full tracks + stems + grids), `goa_crops` (older 11.9 s),
...
inventory sweep: 2026-07-20 (4 agents across mir/SAO). Add new reusable building blocks
here as they land — a stale index is how repeat work happens.
...
- **Meters / scorers (`eval/`)** — `width_metric.py` (canonical stereo width; standard column) · `stereo_phase_meter.py` · `reverb_table_measure.py` · `envelope_fidelity.py` (source-vs-output envelope + pad-fill) · `structure_stability.py` (tempo-IQR/key/recurrence) · `latent_dim_feature_xcorr.py`+`latent_noise_fragility.py` (CPU encodability probe — ENCODED/EMERGENT/DEGRADED) · `latent_bend.py` (deterministic latent ops) · `score_loopy_manifest.py`+`corpus_bands.py` (loop-meter validation + quantile bands) · **`control_head_disintegration_eval.py` — MANDATORY gate before any control-head "works/usable-range" claim** (Kim direct 2026-07-20; every steered clip vs its OWN baseline — dead heads pass, buzz heads can score high authority, a usable head clears both; bounds whitening/hf-blowout/noise/beat-loss/CE-drift; reads `clip_metrics.db`, no re-render; supersedes `latch_bracket_quality_gate.py`. Spec: `docs/superpowers/specs/2026-07-20-control-head-disintegration-gate.md` + MASTER control-eval §) · `drift_prediction_analysis.py`.
...
- **Comms** — `Misc/agent_dialogue.py` (instance-to-instance chat/DM protocol over `osc_worklog.py` multicast; `say`/`dm-say`/`wait`/`listen`) · `Misc/filelock.py` (advisory locks for shared files — **use before editing MASTER.md/specs**; **`--pid-aware` = a long-held-resource mutex** that breaks a foreign lock only when its PID is dead, not on the 15-min mtime timer — the **GPU mutex** — **canonical path `SAO/.gpu.lock` (repo root), the SAME for all instances** — so a live multi-hour job is never stolen but a crashed/rebooted one reclaims instantly. `acquire SAO/.gpu.lock --handle H --pid-aware --pid $$` (the `$$` records the wrapping shell as the persistent holder — NOT the transient CLI) before any render/train/eval, `release` after; prevents the concurrent-GPU overlap that hard-crashed the box twice 2026-07-21) · `worklog_note.sh` (log+notify).
