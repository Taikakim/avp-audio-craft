...
- **Comms** — `Misc/agent_dialogue.py` (instance-to-instance chat/DM protocol over `osc_worklog.py` multicast; `say`/`dm-say`/`wait`/`listen`) · `Misc/filelock.py` (advisory locks for shared files — **use before editing MASTER.md/specs**) · `worklog_note.sh` (log+notify).
