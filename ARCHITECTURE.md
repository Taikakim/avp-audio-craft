...
| **`eval/head_lab.py`** | **the control-head EXPERIMENTATION DRIVER** (Kim 2026-08-23, to kickstart a phase of experimenting with what we have): combine N LatCH heads in one render with per-head `weight` / target `kind:value` / t-window `start:end`, sweep ONE axis with everything else pinned, `--baseline` for the no-guidance control, full resolved config to a JSON sidecar so any result replays. `--list-heads` prints all 19 heads with the defaults their OWN metadata carries (out_channels, trained loss, target kind, standardisation) — note `same_chroma` is 384-ch/cosine and `hpcp` 12-ch while the rest are 1-ch scalars. Drives LatCH guidance ONLY: Head-B control adapters use control-context tokens through a single module-global slot and do NOT compose with each other; see `docs/INFERENCE-SURFACE.md` §4. |
