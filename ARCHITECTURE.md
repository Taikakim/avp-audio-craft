...
> **For "which corpus can train what", read `docs/data.md`** — the generated training-data
> census (every latent store, target store, whole-track timeseries store and database, plus
> how to read each one). This section is the drive map; that one is the *capability* map, and
> it is regenerated by `Misc/training_data_census.py` instead of being maintained by hand.

- **NVMe** (`/home/kim/Projects/`) — the live SA3 latents. `latents_sa3` (14 G; 5401
  `.npy`+`.json`+`.TIMESERIES.npz`, 256-d, T=4096, 10.767 Hz) — the one deliberate
  exception kept here after the 2026-09-23 cleanup below (removable-drive speed wasn't
  judged worth trading for OS-drive reliability on the live training set). Also
  `latents_sa3_subset300` (2.4 M, sole copy, too small to matter). **Backup:** a second
  copy of `latents_sa3` at `/run/media/kim/Mantu/sa3-latents_backup/latents_sa3/`
  (verified in parity 07-12; sync is **manual**, keep it current). `Lehto/latents_sa3`
  was removed 07-04 — do not look there. **Everything else that used to sit on
  `/home/kim/Projects` (~24 latent stores, ~82 G) was deleted 2026-09-23** — all were
  checksum-verified byte-identical duplicates of a copy already on Lehto's
  `latents-all-backup` or on Kosmos (below); see MASTER §2 for the per-store list and
  the one false-alarm divergence (`latents_avp_originals`, resolved — see that note).
- **Mantu** (`/run/media/kim/Mantu`) — source audio + checkpoints + the latents backup.
  ...
  `avp-analyzed-stems` and `suomisoundi_data` moved OFF to Lehto 2026-09-22 (Kim direct)
  — do not look for them here.
- **Lehto** (`/run/media/kim/Lehto`) — training/analysis data, not eval/checkpoints.
  `avp-analyzed-stems`, `suomisoundi_data/` (from Mantu 2026-09-22) · `latents_sa3_lora300`,
  `sa3-latch-latents` (small, untouched) · `latents-all-backup` — now the **sole surviving
  copy** of ~23 control/target latent sets that used to be duplicated on the OS drive
  (`latents_sa3_ctrl`, `latents_sa3_proll`, `latents_sa3_notegrid88`, `latents_sa3_chroma`,
  `latents_avp_ctrl`, the morph variants, `latents_prog_*`, the AVP subsets, `latents_chill`,
  `latents_organic_dance`); `latents_sa3_melody`/`latents_sa3_metrical` also mirrored to
  Kosmos since `sa3_control/train.py` reads them live (dataloader-hot, not just backup).
  `latents`/`latents_stems` (old SAO-Small/SA1 VAE latents) **retired for real 2026-09-23**
  — Kim deleted them; confirmed unusable for SA3 since `profiles/ghost-note.tasks.md`
  2026-05-26, no live code depended on them. `timeseries`, `latents_sa3_stem_chroma` and
  the rest of the bulk precalc/sidecar sets moved to Kosmos 2026-09-22 (below) — Lehto was
  89% full and its I/O-contention issue at MASTER §"first-step hang" predates the move.
- **Kosmos** (`/run/media/kim/Kosmos`) — new 256G NVMe, **dataset precalcs/sidecars**
  (2026-09-22). `timeseries` (37 G, the **46-field** whole-track set, the sidecar
  `field_rates`/`fields`/`expanded_version` meta lives here), `latents_sa3_stem_chroma`
  (29 G), `latents_goa_aug8` (16 G), `suomisoundi_latents` (5 G), `section_labels` — all
  ex-Lehto. `latents_avp` (6 G, ex-NVMe `/home/kim/Projects`), `latents_goa_bigset` (53 G,
  ex-UUID drive — UUID was 96% full, 156G avail; `goa_archive_features` (161 G) stayed
  there, too big for Kosmos's remaining room), `latents_sa3_melody` (11 M) and
  `latents_sa3_metrical` (156 M, dataloader-hot, mirrored from Lehto's `latents-all-backup`
  rather than moved — see Lehto above). Every move `rsync -a`, checksum-verified per file
  (not just size/count) before deleting the source. `Misc/training_data_census.py` carries
  the per-store paths; treat that script's `KOSMOS`/`LEHTO`/`LEHTO_BACKUP` constants as the
  source of truth over this paragraph if they drift.
- **UUID drive** (`/run/media/kim/9a410a1d-a4a8-4faf-8298-bcaa2576ea9d`) — **canonical
  ...
  - **UUID-root `goa_archive_captions/` and `suomisoundi_*` (flagged here 2026-08-18 as a
    same-name duplicate of the `lumi_runs/` copies) are GONE** — resolved sometime between
    then and 2026-09-23 (not by me; verified directly, neither exists at UUID root any more).
  - `goa_archive_captions` — **moved off UUID entirely to Kosmos 2026-09-23** (Kim direct:
    small files, NVMe wins; was at `lumi_runs/goa_archive_captions`). The CURRENT v5
    caption generation
    (`granite_pre_v5_backup/` inside it is the deliberately-kept pre-fix generation, dated
    2026-08-04; the live `granite/` is the 2026-08-17 fix). Checked for symlinks pointing
    in before the move (none) — see the `latents_avp_*` incident note in MASTER §2 for why
    that check matters now. Earlier the same day a STALE duplicate under this same name had
    sat on Kosmos holding the PRE-v5 (contaminated, "1.24× = chance") content — that was
    found by checksum and deleted; the copy now on Kosmos is the verified-correct one that
    came from this UUID path, not a repeat of the stale one.
  - `lumi_runs/goa_archive_captions_hinted/` and `lumi_runs/muscriptor_full/` — moved to
    Kosmos 2026-09-23 (sole copies, no divergence risk, unlike the above). See MASTER §2.
...
|---|---|---|---|
| **Viewer** | `mir` branch **`sa3-latent-explorer`**, `plots/explorer_sa3/app.py` | **8051**, `mir/mir/bin/python` | Dash GUI. Tabs `inference_tab` · `a2a_tab` · `bend_tab`; `render_client.py` is the client, `controls.py` the shared steering panel |
| **Latent player** | `mir/scripts/latent_server_sa3.py` (+ `latent_server_onnx.py`, low-VRAM) | **7892**, `stable-audio-3/.venv` | CROPS ONLY: `/decode /mix /steer`. **Not** the inference path — `/steer` is one head, one gradient step, gain only |
...
- **Fleet-wide checkpoint catalog** (`scripts/analyze_all_checkpoints.py`, 2026-09-21) — same trajectory math as `checkpoint_trajectory_stats.py` above (Frobenius norm, step-to-step velocity/cosine, path efficiency) but scoped differently: ONE resumable sweep across every checkpoint on every mount (`sa3_lora_runs`, `lumi_runs`, `sa3_control_runs`), streamed to `checkpoint_analysis_catalog.jsonl` + `checkpoint_analysis_summary.{md,csv}` at the repo root, rather than one `--ckpt-dir` at a time. Use this for "how does run X compare to everything else", the per-run tool above for "how is run X doing on its own". Both landed together in the 2026-09-21 VADD/discontinuity report (`OPTIMIZER_TRAJECTORY_LATENT_AND_DISCONTINUITY_FINDINGS.md`, repo root).
...
- **MIDI features in the metrics DB (`eval/midi_metrics_ingest.py`, G 2026-09-09)** — folds `hook_eval_renders.py`'s MuScriptor transcription features into `clip_metrics.db` as a SEPARATE `midi_metrics` table keyed by `path` (not new columns on `metrics`: other tools `SELECT *` and build their column list from it). Worth joining because these features are **orthogonal to the aesthetic metrics**: on 146 clips, whether a render contains a melodic lead at all is invisible to PQ (d −0.10, z −0.61) and to CE (d +0.22, z +1.27), and only partly visible to crest (d +0.45, z +2.62). ⚠ Coverage is the catch — `has_lead`/`n_lead` are defined for every clip, but the melodic features (hook_melodic_ratio, contour_compression, top_motif…) are NULL on ~66% because 40% of renders have no lead voice at all; averaging them over a mixed set averages over "no melody" as if it were missing at random.
...
- ⚠ **`scripts/render_standard_eval_grid.py` is NOT the canonical renderer — `eval/model_matrix_gen.py` is (see `sa3-canonical-clips` skill).** It's a one-off, self-contained 72-cell grid renderer (12 canonical prompts × cfg {7,16} × strength {1,1.5,2}) built for the 2026-09-21 VADD/discontinuity report to compute inline DSP metrics (crest factor, spectral centroid/flatness via Welch PSD, discontinuity jumps) on its own renders — never wired to `clip_metrics.db` or the board builders, and its default `--ckpt` is hardcoded to one specific 2026-09-21 run. Not worth re-running as-is; **its one genuinely new metric (crest factor = peak/rms, a real squashing/over-guidance signal per MASTER.md §5's target_raw section) was ported into `eval/demo_clip_scan.py` 2026-09-25** rather than kept as a separate tool. The report's actual published discontinuity numbers used the existing `eval/audio_corruption_scan.py`, not this script.
- **Resident adapter slots / A-B (`eval/adapter_slots.py` + `/slots` + `POST /ab`, C 2026-08-26)** — hold N adapters resident on ONE base and switch between them in milliseconds, instead of a multi-GB reload per model. `SlotTable.plan()` is pure (refuses a set that would cross the configured `vram_floor_gb` rather than OOMing mid-render, and refuses a `fullft` family outright — it replaces the backbone, it cannot share it); `.apply()` does remove-all + ONE `load_lora` because `load_and_apply_loras` re-indexes from 0; `.lora_configs()` ALWAYS covers every resident index, because `dit.py` only touches indices present there and an omitted one keeps its last enable state — that is how "A vs B" silently becomes "A+B". **MEASURED on the card, both numbers correcting the plan's estimates:** medium-base leaves 7.40 GB free (baseline ~8.5 GB, not ~5.1), and an r128 DoRA costs **0.40 GB** not 0.33 — `model_db._GB_PER_RANK_HALF` was corrected. `remove_lora_by_index` + reload reverts the base weights EXACTLY (max drift 0.0 over 50 tensors), which is what makes set changes safe. `POST /ab` renders one payload across arms with ONE shared seed (`null` = the bare base, the control arm). Viewer: an "A/B slots" panel in the inference tab. Verified live: slot 0 / slot 1 / base all distinct, `rebuilt=False` throughout, and returning to slot 0 reproduces its first render byte-for-byte. Tests `test_adapter_slots.py`, `test_render_server_slots_api.py`, `test_render_server_ab.py`.
- **Batch/sweep surface (`eval/sweep_spec.py` + `sweep_run.py` + `build_sweep_page.py`, C 2026-08-26)** — the answer to "sweep one axis with everything else pinned", which a GUI is the wrong shape for (gap #4 in `docs/INFERENCE-SURFACE.md`). `sweep_spec` is the pure grammar: axis name → dotted payload path (`AXIS_KINDS`, and any raw dotted path works, so it needs no edit when the server grows a knob), cross-product expansion, and a `cell_id` that is a hash of the canonical coords — **the resume key**, so it survives typing the axes in a different order and distinguishes floats that format identically. `sweep_run.py` is the resumable CLI and **a THIN CLIENT of :8056** — it never builds a guidance config, because `resolve_latch()` normalises gains and a second implementation would silently be on another scale (`eval/head_lab.py` was deleted for exactly that). One fsynced JSON line per cell in `manifest.jsonl`, storing the payload VERBATIM; errors are retried on rerun, successes are not. `--models id1,id2` renders one preset across models from the model DB, failing loudly on an unresolved id BEFORE any render and keying `cell_id` on the model LABEL so a remount does not re-render a finished sweep. `build_sweep_page.py` turns the manifest into a three-audience page per the eval-tables spec, reusing `build_evals`' `CSS`/`WAVEFORM_CSS`/`WAVEFORM_JS` same-playhead player (loaded BY PATH — `build_evals.py:21` prepends `Misc/` to `sys.path`, where a first-party `filelock.py` shadows the pip package). Example spec in `eval/sweeps/`. Tests: `test_sweep_spec.py`, `test_sweep_run.py`, `test_build_sweep_page.py`.
- **Continuation provenance (`eval/continuation.py`, C 2026-08-26)** — `recover(z0_path)` reads a prior render's `ckpt_path`/`dora` from whichever sidecar sits next to its latent (`result.json` from :8056, or `<stem>.mmline.json` from the matrix-cell renderers); `apply(req, prior)` makes a continuation that specifies nothing INHERIT the prefix's model rather than whatever the server has resident, and turns a deliberate divergence into a named warning instead of a silent one. Wired into `/longform init_latent_path`. Note the caller always wins, and a conflicting `ckpt_path` suppresses DoRA inheritance — an adapter belongs to its own base. Tests `eval/tests/test_continuation_provenance.py`.
- **Render presets (`eval/presets.py`, C 2026-08-26)** — a preset is a `/generate` payload with a name, stored under `eval/presets/<slug>.json`, served by `GET/POST /presets` and `GET /presets/<name>` on :8056. **`seed` and `batch_size` are stripped on save** — a preset is a RECIPE, and a pinned seed would make a sweep's seed axis a silent no-op. Alongside the server payload it stores an optional **`form` snapshot** (`{dash_id: value}`) so the viewer restores its boxes and re-derives the payload through the SAME `build_payload()` rather than an inverse mapping that would drift. Verified round-trip: 63/63 non-volatile fields, and the rebuilt payload is byte-equal to the stored one. Viewer row + two-phase Load in `mir/plots/explorer_sa3/inference_tab.py`; client `render_client.presets_list/preset_load/preset_save`. Tests `eval/tests/test_presets.py` + `mir/tests/explorer_sa3/test_preset_form_partition.py`. This is the substrate the sweep CLI (plan Tasks 6-9) consumes — a sweep is a preset plus axes.
- **LatCH head metadata (`eval/head_meta.py`, C 2026-08-26)** — `describe()` turns a LatCH checkpoint's own `std_mean/std_std/epoch/loss_type/out_channels` into the honest head description the inference UI shows: **per-head slider bounds = dataset mean ± 2σ in RAW feature units** (the server used to hand every head the same -80/20/-30, right for the dB-valued `rms_*` family and meaningless for `beat_activation` 0.044±0.066 or `hardness` 66.2±3.5), capability flags (`supports_scalar_target` False for the 384-ch `same_chroma` and 12-ch `hpcp`; `supports_loss_select` False for a cosine objective), a health flag (`spectral_kurtosis` = undertrained, epoch 3, σ 521.6) and readout provenance. Human-supplied facts the checkpoints don't carry live in `Misc/latch_head_overrides.json`. Consumed by `explorer_render_server._head_entry` → `/info` → `controls._slot_view` in the viewer. Tests `eval/tests/test_head_meta.py` + `test_server_heads.py`.
...
- **LoRA-TSD optimizer + gradient flight recorder (C, 2026-09-25)** — `stable-audio-tools/stable_audio_tools/training/lora_tsd/` (`BatchedLoRATSD`, arXiv 2609.02734: steps sized on B·A; `reference.py` oracle, `bench.py`, `NOTES.md`; tests `stable-audio-tools/tests/test_lora_tsd_batched.py`), used via `train_lora_modular.py --optimizer lora_tsd` (manual §5e). · `stable-audio-3/stable_audio_3/training/flight_recorder.py`: raw pre-clip grad norms per kind (`grad/raw_norm_*`) plus incident dumps on a spike or a non-finite grad to `<run>/incidents/` (`--flight-recorder`, default on; manual §7); read them with `eval/inspect_flight_incident.py <run_dir>`.
...
- **Duplicate-checkpoint reclaim** — `Misc/dedupe_model_copies.py`. Finds parallel copies of the same checkpoint across the eval drives (the LUMI tree was rsynced more than once with different path prefixes: `Mantu/lumi_runs/<arm>/`, `Mantu/lumi_runs/runs/runs/<arm>/`, `<UUID>/lumi_runs/runs/<arm>/`). Dry-run by default; `--hardlink` keeps every path (same filesystem only), `--apply` deletes; both require `--verify full`. **Groups on the trailing path components, NOT the filename** — sibling arms (`fullft_avp_t256` vs `t1024`) share a name AND a byte count while being different models, and keying on the filename overstates the waste ~4x and would delete unique weights. First run 2026-08-25: 298 groups, 295 GB reclaimable (48.5 GB of it intra-Mantu and hardlinkable).
...
| **`RUNBOOK.md`** | **the OPERATOR manual — every routine operation as a copy-pasteable command** (cwd, absolute venv path, required exports, expected wall-clock, and the artifact-level VERIFY step). Written for Kim to run the machine with no agent in the loop, because the token budget runs out by mid-week and the lab must not stop with it (Kim direct 2026-09-08). Agents: fix it the same session you change a script's interface, and link to its section from `KIM-TASKLIST.md` instead of retyping an invocation. Rules: `CLAUDE.md` §8, `MASTER.md` §8 |
| `KIM-TASKLIST.md` | team-maintained — the single place the fleet surfaces what needs Kim (decisions, ears, reviews, submits); add when work lands that needs him, move to Recently done when resolved; THE-FINN patrols for staleness (see `.claude/skills/weekly-routine/SKILL.md`) |
| **`.claude/skills/weekly-routine/SKILL.md`** | the recurring cadence: dialogue-chronicle synopsis, public blog week-in-review, KIM-TASKLIST staleness patrol, EXPERIMENTS sanity pass — run when a week closes or Kim asks to "compile the weeklies" (added 2026-08-28/09-01 after this lapsed unnoticed for several weeks) |
...
| **`docs/INFERENCE-SURFACE.md`** | **the CALLABLE surface, written for a UI builder** (Kim 2026-08-23: "document the code well, I will then create a UI"): every generation entry point (t2a / a2a / longform / control), how to TELL THE FOUR CHECKPOINT FAMILIES APART (base · adapter · full-FT · Head-B control, dispatched on ckpt shape, plus the `riffer_final.pt`-vs-`epoch=*.ckpt` naming split), the FOUR independent control mechanisms (text+cfg · adapter strength · LatCH guidance configs + the head inventory · control-context tokens), the in/out file conventions, and a traps list where each item produces plausible output rather than an error (the 120 s `sample_size` default, control loaded but never applied, `refs/` dirs that are NOT model output). Ends with the honest fragmentation list behind Kim's unification ask — **a single conditioning inlet is the prerequisite**, and it is why D14 had to drop the pianoroll arm. |
| **`eval/explorer_render_server.py`** | **THE INFERENCE ENGINE — the resident render server the latent-explorer GUI drives.** FastAPI on :8056, `medium-base` held resident on the GPU. Endpoints `/info /status /ckpts /audio /generate /a2a_track /a2a_mix /longform /decode /bend`. `/generate` already accepts a **LIST of LatCH slots** (`resolve_latch`): per-slot head/path · kind · value · start_pct/end_pct · gain · loss_type · w_sec, plus parameterless builtins, against a `HEADS` registry with per-head defaults and server-side gain normalisation — **so multi-head combinations with tunable values already work here**. Also carries DoRA + FiLM adapter install, weight mutation, negative prompt, cfg_interval, dist_shift. Render logic is IMPORTED from the proven eval scripts (`chroma_morph_transitions.py`, `a2a_fulltrack.py`, `density_control_eval.py`), not re-derived. GUI: mir branch `sa3-latent-explorer`. **Check here before building any new inference surface** — a parallel CLI was built and deleted on 2026-08-23 for exactly this reason. |
| **`docs/data.md`** | **the TRAINING-DATA CENSUS — every corpus, target store and database, with what each one can actually train** (Kim's ask 2026-09-16: "agents look for this information all the time"). Generated by `Misc/training_data_census.py`, so **re-run it** rather than trusting the counts. Answers "which corpora have latents vs scalars vs per-frame targets", and defuses two naming traps that have each already cost a session: **`.npy` is always the latent, `.npz` is always a target/companion** (a store can be 100% `.npz` and train nothing), and **several TARGET stores are named `latents_*`** (`latents_sa3_chroma`, `latents_sa3_stem_chroma`, `latents_sa3_proll`). Also carries the access snippets — `TimeseriesDB`, the correct whole-track consumer, the `f__`/`r__` prefix layout, `clip_metrics.db` — and the dataloader trap behind them (the 100 Hz rate assumption in `whole_track_target_source.get()` that silently DROPS crops carrying a sub-100 Hz field, rather than erroring). Also states which fields each whole-track store actually holds — goa_archive is FULL-TRACK but carries the 24 expanded fields ONLY, so an onset/rms head needs the base extractor run over it first. |
...
| **`docs/render-analyze-publish.md`** | **the render→score→publish walkthrough** — register an arm, render the canonical clip set, score it, and rebuild the local board pages, in order, with exact commands and where each output actually lands (added 2026-09-25). The command-of-record copy lives in `RUNBOOK.md` §7/§12; this is the narrative version. |
| **`docs/train_lora_modular.md`** | **operator manual for `train_lora_modular.py`: every flag, default, tried values and expected effect; known-good recipes; do-it-yourself analysis** — clip scan, trajectory (velocity + path-efficiency, with reference numbers), LoRA A/B gauge drift, a DSP disintegration spot-check, and a while-it's-still-running triage variant (§8/§8b, extended 2026-09-25) — **works on any run dir, not just `train_lora_modular.py`'s own** (Kim 2026-09-23). Kept in sync by `Misc/check_train_manual.py` |
...
| **`eval/build_morph_page.py`** | **the morph-conditioner audition page** (Kim 2026-09-03 "create the clips and an eval site") — 384 rendered cells over 16 arms × 8 source stems × {control-off, gain 1.0, gain 2.0}, plus each stem's REFERENCE clip, as one same-playhead table per stem. Reuses `Misc/build_evals.py` PLAYER_JS rather than writing a third player (same rule as `build_sweep_page.py`), and loads it BY PATH because build_evals:21 poisons `sys.path`. Output `~/evals_aac/morph_conditioner/` (AAC 192k, matching the serving codec). The clips already existed on the UUID drive and had never been auditioned. ⚠ Listening surface ONLY — no objective adherence metric has been run on the contour streams yet. |
| **`eval/soup_pt_ladder.py`** | **dial on "how much post-training"** — `W = base + a*(PT-base)` between `stable-audio-3-medium` (PT) and `-medium-base`, with `--targeted-alpha/--targeted-regex` to hold a subset at a different alpha. Kim 2026-09-04 (PT is punchy+coherent but "always sounds more or less the same with the kick, bass and percussions"). **⛔ FALSIFIED BY EAR 2026-09-07 (Kim): PT does NOT blend with base — every rung is degraded, only α=0.5 is even listenable and it still has artifacts. Do not build this ladder again.** The assumption below was that a fine-tune shares one trajectory with its base and so sits in the linear-mode-connectivity regime, unlike the independently-trained soups of C1/C2 — that reasoning did not survive contact with the audio, and post-training moves the model out of the regime despite being a fine-tune. Retained only as the record of a closed negative: measured change is CONCENTRATED not small (median rel delta 0.0013 but mean 0.0215, max 0.714, in `to_local_embed` biases L16-19 — G measured the functional tail as hf_ratio +57% on rendered clips). Endpoints self-verify exactly. 8.6 GB/blend — build, render, delete. |
| **`eval/render_soup.py`** | **renders a soup blend — or an unblended ENDPOINT** (`--endpoint {base,pt}`, loading from the registered name rather than building two more 8.6 GB blend dirs that would duplicate models already on disk; without endpoints the blends have nothing to be *between*). `--sampler` is load-bearing, not a convenience: `medium` is `diffusion_objective: rf_denoiser` (native **pingpong**), `medium-base` is `rectified_flow` (**euler**), and a blend loads medium-base's CONFIG whatever its alpha — so every blend samples as rectified_flow and `ptm_local000` is PT weights under the base's objective. Render both samplers; compare only WITHIN one. ⚠ Two traps: pingpong fragments the allocator far worse than euler and OOMs every arm on a 16 GB card unless `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`; and renders **segfault at teardown AFTER writing every file**, so the exit code lies — count the wavs. |
| **`eval/soup_descriptors.py`** | **does an intermediate alpha land BETWEEN the endpoints, or off-manifold?** Reuses `disintegration_metrics.measure()` (no parallel descriptor set) and **self-gates per audit-the-instrument-first**: a descriptor is reported only if it separates the endpoints by more than the within-endpoint prompt spread — 6 of 8 cells came back MUTE on the first run, which is the point. Scores no quality: "punchy but varied" is Kim's ear, not a number here. **⛔ MOOT — Kim's ears closed C6 as a negative 2026-09-07: the blends are DEGRADED, not interpolated, so these descriptors were reading artifact spectra rather than a mixture. That is why the two samplers disagreed about the same 48 tensors — degradation has no reason to be consistent across sampling objectives, and 'between the endpoints in descriptor space' never implied musically between them.** Superseded reading kept for the record: Verdict (n=3/cell, directional): **localization is UNRESOLVED — the samplers disagree about the same 48 `to_local_embed` biases.** Holding them at base recovers **9%** of the change under euler/8 (`hf`) but **87–88%** under pingpong/24 (`flatness`, `zcr`). That is not noise: neither table is a clean weights-only comparison (euler runs PT off its native sampler, pingpong runs base off its), so each measures both models under ONE model's objective, and a local-conditioning bias plausibly matters differently under a different denoiser objective. What holds: α=0.5 interpolates monotonically wherever a descriptor is readable, and at euler/24 both blends land OUTSIDE the endpoint range ⇒ linear interpolation leaves the audio manifold there. |
| **`eval/build_soup_page.py`** | **the soup-ladder audition page** — arms down the rewind (base → α0.5 → α1-minus-biases → PT) × prompt × steps, same-playhead, **one table PER SAMPLER** (it refuses to mix them, per the objective mismatch above). Reuses `Misc/build_evals.py` PLAYER_JS + `redact()` BY PATH (build_evals:21 poisons `sys.path`). Carries the three-audience explainer. `--public` writes the only page that may go online. Output `~/evals_aac/soup_rewind/`. |
| **`eval/morph_merit.py` · `eval/morph_note_ab.py` · `eval/morph_contour_ab.py`** | **morph-conditioner adherence, three instruments, ALL currently failing their gates** — MERIT S_mel (saturated on same-corpus material), D15 transcription (no own-vs-foreign discrimination), contour-to-contour (fails its positive control; encoder reaches only 0.55 on L3 against ground truth). Each carries its control as a FATAL gate. ⚠ Read the gate output before quoting any number from these. |
